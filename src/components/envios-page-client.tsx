...
import { createEnvioAction, deleteEnvioAction, updateEnvioAction } from "@/app/dashboard/actions";
import { EnvioDrawer, NewEnvioButton } from "@/components/new-envio-button";
import { OperatorChip } from "@/components/operator-chip";
import type { Envio } from "@/lib/data";
import { fmtAWG, fmtCOP, fmtDate, fmtDayLabel, fmtNum, fmtUSD } from "@/lib/formatters";
import type { EnvioRecord } from "@/lib/movements-data";

const OPERATORS = ["Todos", "ROYMAN", "ERIKA", "LINA", "JUAN PABLO"] as const;

export function EnviosPageClient({ initialRows }: { initialRows: EnvioRecord[] }) {
  ...
  const updateEnvio = async (id: number | string, envio: Envio) => {
    ...
    setRows((current) => current.map((row) => (row.id === id ? { ...row, ...envio } : row)));
  };
    ...
    <>
      <div className="topbar">
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
          ...
          <NewEnvioButton label="+ Nuevo envío" style={{ fontSize: 12 }} onSave={createEnvio} />
        </div>
      </div>
      ...
      <div className="content" style={{ padding: "28px 32px", flex: 1, overflowY: "auto" }}>
        ...
        >
          {[
            { label: "Registros filtrados", value: String(filtered.length), mono: true },
            { label: "Total pesos filtrado", value: fmtCOP(filteredPesos) },
            { label: "Total florines filtrado", value: fmtAWG(filteredFlorines) },
            { label: "Ganancias filtradas", value: fmtCOP(filteredGan), accent: true },
          ].map((s) => (
            <div
              key={s.label}
            ...
            >
              ...
              <div className={`value ${s.mono ? "mono" : "serif"}`} style={{ fontSize: 22 }}>
                {s.value}
              </div>
            </div>
          ))}
          ...
          >
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button
                ...
                style={{ fontSize: 12, opacity: currentPage === totalPages ? 0.45 : 1 }}
                type="button"
              ...
              <tbody>
                {sortedDays.length === 0 && (
                  <tr>
                    <td
                      colSpan={10}
                      style={{ textAlign: "center", padding: "32px 0", color: "#5a5f68" }}
                    >
              ...
              {currentDayRows.length > 0 && (
                <tfoot>
                  <tr style={{ borderTop: "1px solid #2a2f38" }}>
                    ...
                    <td className="num" style={{ fontWeight: 600, color: "#e8eaed" }}>
                      {fmtCOP(totalPesos)}
                    </td>
                    <td className="num" style={{ fontWeight: 600, color: "#e8eaed" }}>
