...
Next.js 15+ uses ESLint 9 flat config (`eslint.config.mjs`). The `eslint-config-next` package ships:

- `eslint-config-next` — base config (React, react-hooks, jsx-a11y, import, @next/eslint-plugin-next)
- `eslint-config-next/core-web-vitals` — stricter Core Web Vitals rules (recommended)
...
`eslint-config-prettier` disables ESLint rules that conflict with Prettier. Do **not** install `eslint-plugin-prettier` — it runs Prettier as a lint rule (slow, noisy in editor). Instead, run Prettier as a separate step.
...
### lint-staged order (package.json)

```json
"lint-staged": {
  "*.{js,jsx,ts,tsx}": ["prettier --write", "eslint --fix"],
  "*.{json,md,css,mjs,cjs}": ["prettier --write"]
}
```

Prettier first → ESLint second. This ensures formatting is applied before linting fixes.
